...
export class GraphQLPageClient {
  ...
  async getAllToolInfo() {
    const queryAdventuresAll = 'maps/pacs-header';
    const res = await this.aemHeadlessClient.runPersistedQuery(queryAdventuresAll);
    return res;
  }

  async getValuesPaths() {
    const res = await this.getAllToolInfo();
    const adventures = res?.data?.sDCToolList?.items || [];
  ...
  async getValuesByPath(path) {
    const query = `{
      pacsDescriptionList (_path: "${path}") {
        items {
          _path,
          title,
          description {
